...
{	 
	...
	while(i < 32){        //每个数字，取32次ADC度数的LSB组成的无符号整型，保证随机性
	  while(!ADC_GetFlagStatus(ADC1, ADC_FLAG_EOC));//等待转换结束
	  val |= ((ADC_GetConversionValue(ADC1) & 0x0001) << i);	//返回最近一次ADC1规则组的转换结果
	  ADC_ClearFlag(ADC1,ADC_FLAG_EOC);
		++i;
	}
	
	ADC_SoftwareStartConvCmd(ADC1, DISABLE);		//禁能ADC1软件转换
